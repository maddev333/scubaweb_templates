...
#Assign Role for Exchange
    New-MgServicePrincipalAppRoleAssignment -ServicePrincipalId $ManagedIdentity.Id `
    -PrincipalId $ManagedIdentity.Id -ResourceId $ExID -AppRoleId $getExPerms.id


# Assign Global Reader in Tenan
$roleId = (Get-MgRoleManagementDirectoryRoleDefinition -Filter "DisplayName eq 'Global Administrator'").id
New-MgRoleManagementDirectoryRoleAssignment -PrincipalId $ManagedIdentity -RoleDefinitionid $roleid -DirectoryScopeid "/"
